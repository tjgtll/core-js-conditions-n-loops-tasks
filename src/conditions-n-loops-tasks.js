  ...
  for (let i = digits.length - 2; i >= 0; i -= 1) {
    if (digits[i] < digits[i + 1]) {
      ...
      for (let j = i + 1; j < digits.length; j += 1) {
        if (digits[j] > digits[i] && digits[j] <= digits[largerIndex]) {
          largerIndex = j;
        }
      }

      const temp = digits[i];
      digits[i] = digits[largerIndex];
      digits[largerIndex] = temp;
